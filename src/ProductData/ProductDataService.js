...
export default class ProductDataService {
    ...
    static filterQuery(str,filterObj={},maxResults=10){
        str = str.toLowerCase();
        return (ProductDataService.productData.products
            .filter(product=>product.name.toLowerCase().indexOf(str)>-1)
            .slice(0,maxResults)
        )
    }

    static matchFirst(str,filterObj={},maxResults=10){
        str = str.toLowerCase();
        ...
        if (filterKeys.length >0) {
            ...
            return (ProductDataService.productData.products
                .filter(product=>product.name.toLowerCase().indexOf(str)===0
    ...
    static get productData() {
        let PDS = ProductDataService;
        if (PDS._uniqueProductData.products.length === 0) {
           let uniqueProducts = [];
           PDS._productData.products.forEach( (product) => {
               let found = false;
               for ( let foundProduct of uniqueProducts ) {
               ...
               if (!found) uniqueProducts.push(product);
           } )
           PDS._uniqueProductData.products = uniqueProducts;
        }
        return PDS._uniqueProductData;
    }

    static _uniqueProductData = { products:[] }
    static _productData = {
    "products": [
        {
